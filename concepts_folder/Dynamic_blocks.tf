...
variable "allowed_ports" {
  ...
  default = [8080, 8081, 22, 443 ] # Default ports allowed for incoming traffic
}
...
resource "aws_security_group" "Test-sg" {
  ...
  dynamic "ingress" {
    ...
    content {
      from_port   = ingress.value # The starting port for the ingress rule
      to_port     = ingress.value # The ending port for the ingress rule
